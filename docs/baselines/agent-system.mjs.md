...
- `validate` checks the active profile, status files, change files, training logs, evaluation logs, backup schema, quick-update flow, quick-fix flow, luau-quick flow, upgrade flow, and host memory layout.
- `lint` enforces manifest consistency, memory drift checks, status presence, change presence, pack presence, training presence, evaluation presence, backup schema presence, quick-update coverage, quick-fix coverage, luau-quick coverage, upgrade coverage, and training coverage.
...
- `train` auto-detects Luau work and writes Luau-focused lessons into the training log when the active change is Luau-aware.
- `eval` auto-detects Luau work and writes Luau-focused lessons into the evaluation log when the active change is Luau-aware.
